...
driver.find_element_by_xpath("//img[@class='rx-logo']").is_displayed()

print("Displays")
driver.find_element_by_xpath("(//span[@class='mat-button-wrapper'][contains(.,'Clinical Intervention Category')])[1]").click()

driver.find_element_by_xpath("//button[contains(.,'Add New Category')]").click()
time.sleep(5)
driver.find_element_by_xpath("//input[@id='txtname']").send_keys(category_name)
time.sleep(5)
driver.find_element_by_xpath("//span[contains(@class,'mat-select-placeholder ng-tns-c16-11 ng-star-inserted')]").click()
clinicaltype_element = WebDriverWait(driver, 15).until(EC.presence_of_element_located(
    (By.XPATH, "//span[@class='mat-option-text'][contains(.,'Clinical Interventions')]")))
time.sleep(5)
...
def Verify_Dupulicate_values(self):
    ...
    driver.find_element_by_xpath(
        "//span[contains(@class,'mat-select-placeholder ng-tns-c16-11 ng-star-inserted')]").click()
    clinicaltype_element = WebDriverWait(driver, 15).until(EC.presence_of_element_located(
        (By.XPATH, "//span[@class='mat-option-text'][contains(.,'Clinical Interventions')]")))
    driver.execute_script("arguments[0].click();", clinicaltype_element)
    time.sleep(5)
